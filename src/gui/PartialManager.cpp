...
#ifdef WIN32_DEBUG
void PartialManager::rawVerifyState( char *name, Synth *useSynth )
{
	PartialManager *ptr1, *ptr2;
	PartialManager partialManager_temp(synth, parts);
	...
	ptr1 = this;
	ptr2 = &partialManager_temp;
	for( int lcv=0; lcv<synth->getPartialLimit(); lcv++ )
		delete ptr2->partialTable[lcv];
	useSynth->rawLoadState( name, ptr2, sizeof(*this) );


	if( ptr1->synth != ptr2->synth ) __asm int 3
	if( ptr1->parts != ptr2->parts ) __asm int 3
	
	for( int lcv=0; lcv<synth->getPartialLimit(); lcv++ )
		if( ptr1->partialTable[lcv] != ptr2->partialTable[lcv] ) __asm int 3

	for( int lcv=0; lcv<9; lcv++ )
		if( ptr1->numReservedPartialsForPart[lcv] != ptr2->numReservedPartialsForPart[lcv] ) __asm int 3
...
void PartialManager::saveState( std::ostream &stream )
{
	...
	stream.write(reinterpret_cast<const char*>(&numReservedPartialsForPart), sizeof(numReservedPartialsForPart) );
...
void PartialManager::loadState( std::istream &stream )
{
	...
	stream.read(reinterpret_cast<char*>(&numReservedPartialsForPart), sizeof(numReservedPartialsForPart) );
